...
// Tango Word Game Implementation
class TangoWordGame {
    constructor() {
        this.letters = [];
        this.availableWords = [];
        ...
        // Common word list for validation
        this.wordList = [
            'cat', 'dog', 'run', 'sun', 'fun', 'gun', 'cut', 'but', 'put', 'get',
            'set', 'let', 'met', 'net', 'pet', 'wet', 'hot', 'not', 'got', 'lot',
            'eat', 'bat', 'rat', 'hat', 'mat', 'fat', 'sat', 'car', 'bar', 'jar',
            'star', 'cart', 'part', 'heart', 'start', 'smart', 'chart', 'great',
            'earth', 'water', 'after', 'under', 'other', 'right', 'light', 'night',
            'think', 'thing', 'bring', 'string', 'strong', 'wrong', 'along', 'song',
            'long', 'among', 'young', 'sound', 'found', 'round', 'ground', 'around'
        ];
    }
    ...
    generateLetters() {
        // Generate 8 random letters with some vowels guaranteed
        const vowels = 'AEIOU';
        ...
        for (let i = 0; i < 2 + Math.floor(Math.random() * 2); i++) {
            this.letters.push(vowels[Math.floor(Math.random() * vowels.length)]);
        }
        
        // Fill with consonants
        while (this.letters.length < 8) {
            this.letters.push(consonants[Math.floor(Math.random() * consonants.length)]);
        }
    ...
    findPossibleWords() {
        ...
        for (let word of this.wordList) {
            if (this.canFormWord(word.toUpperCase())) {
                this.availableWords.push(word.toUpperCase());
            }
        }
    }
    ...
    renderGame() {
        // Render letters
        const lettersElement = document.getElementById('tangoLetters');
        if (lettersElement) {
            lettersElement.innerHTML = this.letters.map(letter => 
                `<div class="letter-tile">${letter}</div>`
            ).join('');
        }
        
        // Update score and timer
        document.getElementById('tangoScore').textContent = `Score: ${this.score}`;
        document.getElementById('tangoTimer').textContent = this.formatTime(this.timeLeft);
        
        // Render found words
        const wordsFoundList = document.getElementById('wordsFoundList');
        const wordsFoundCount = document.getElementById('wordsFoundCount');
        
        if (wordsFoundList) {
            wordsFoundList.innerHTML = this.foundWords.map(word => 
                `<span class="word-item">${word}</span>`
            ).join('');
        }
        
        if (wordsFoundCount) {
            wordsFoundCount.textContent = this.foundWords.length;
        }
    }
    ...
    submitWord() {
        const input = document.getElementById('tangoWordInput');
        ...
        if (!this.canFormWord(word)) {
            alert('You cannot form that word with the available letters!');
            input.value = '';
            return;
        }
        ...
        if (this.foundWords.length === this.availableWords.length) {
            this.endGame(true);
        }
    }
    ...
    endGame(allFound = false) {
        ...
        const result = {
            won: allFound || this.foundWords.length > 0,
            score: this.score,
            wordsFound: this.foundWords.length
        };
        
        window.gamesManager.onGameComplete('tango', result);
        
        const message = allFound ? 
            `Congratulations! You found all ${this.availableWords.length} words!` :
            `Time's up! You found ${this.foundWords.length} out of ${this.availableWords.length} words.`;
    ...
    setupEventListeners() {
        const input = document.getElementById('tangoWordInput');
        const submitBtn = document.getElementById('tangoSubmitBtn');
        ...
        document.getElementById('tangoShuffleBtn').onclick = () => this.shuffleLetters();
        document.getElementById('tangoNewBtn').onclick = () => this.startGame();
    }
}
